mk_inds <- function(covars, gr) {
    lapply(seq_along(gr), function(x) covars[, which(group == gr[x])])
}

ext_mat_files <- function(net, dataCTRL, dataTMS) {
    list(r_c = c(str_subset(
        list.files(list.dirs(paste(dataCTRL, net, sep = '/'), recursive = T),
                          paste("*_matrix", net, "r.csv", sep = '_'), full.names = T),
        paste(str_replace_all(str_subset(covars_p$Study.ID, "ctr-"), 
                              "ctr-", "sub-"), collapse = '|')),
        list.files(list.dirs(paste(dataTMS, net, "all", sep = '/'), recursive = T),
                   paste("*t0_matrix", net, "r.csv", sep = '_'), full.names = T)),
        r_t0 = c(list.files(list.dirs(paste(dataTMS, net, "sham", sep = '/'), recursive = T),
                           paste("*t0_matrix", net, "r.csv", sep = '_'), full.names = T),
                 list.files(list.dirs(paste(dataTMS, net, "tx", sep = '/'), recursive = T),
                            paste("*t0_matrix", net, "r.csv", sep = '_'), full.names = T)),
         r_t1 = c(list.files(list.dirs(paste(dataTMS, net, "sham", sep = '/'), recursive = T),
                             paste("*t1_matrix", net, "r.csv", sep = '_'), full.names = T),
                  list.files(list.dirs(paste(dataTMS, net, "tx", sep = '/'), recursive = T),
                             paste("*t1_matrix", net, "r.csv", sep = '_'), full.names = T)),
         r_tx_pre = c(str_subset(
             list.files(list.dirs(paste(dataTMS, net, "sham", sep = '/'), recursive = T),
                                 paste("*t1_matrix", net, "r.csv", sep = '_'), full.names = T),
             paste(covars2$Study.ID, collapse = '|')),
             list.files(list.dirs(paste(dataTMS, net, "tx", sep = '/'), recursive = T),
                                 paste("*t0_matrix", net, "r.csv", sep = '_'), full.names = T)),
         r_tx_post = c(list.files(list.dirs(paste(dataTMS, net, "sham", sep = '/'), recursive = T),
                             paste("*t14_matrix", net, "r.csv", sep = '_'), full.names = T),
                       list.files(list.dirs(paste(dataTMS, net, "tx", sep = '/'), recursive = T),
                             paste("*t1_matrix", net, "r.csv", sep = '_'), full.names = T)),
         r_t2 = c(list.files(list.dirs(paste(dataTMS, net, "sham", sep = '/'), recursive = T),
                             paste("*t2_matrix", net, "r.csv", sep = '_'), full.names = T), 
                  list.files(list.dirs(paste(dataTMS, net, "tx", sep = '/'), recursive = T), 
                             paste("*t2_matrix", net, "r.csv", sep = '_'), full.names = T)))
}
